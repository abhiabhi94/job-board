...
def test_very_old_jobs_are_skipped(parser):
    ...
    parser_class.get_link = lambda self: self.item["link"]
    parser_class.get_posted_on = lambda self: self.item["posted_on"]
    parser_class._get_job = lambda self: self.item

    portal = BasePortal()
    portal.parser_class = parser_class
