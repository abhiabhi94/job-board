...
class Wellfound(BasePortal):
    ...
    async def _fetch_all_pages(self) -> list[dict[str, Any]]:
        ...
        for i in range(0, len(remaining_pages), self._REQUEST_BATCH_SIZE):
            batch_pages = remaining_pages[i : i + self._REQUEST_BATCH_SIZE]
            batch_urls = [f"{self.url}?page={page}" for page in batch_pages]

            logger.info(
                (
                    f"[Wellfound]: Fetching batch {i // self._REQUEST_BATCH_SIZE + 1}, "
                    f"exc_info = pages {batch_pages[0]}-{batch_pages[-1]}"
                )
            )
            # Fetch batch concurrently
            batch_contents = await asyncio.gather(
                *[self._make_request(url) for url in batch_urls]
            )

            # Process batch results
            for page_num, content in zip(batch_pages, batch_contents):
                graph_data = self._parse_page_content(content)
                jobs_data.append(graph_data)
